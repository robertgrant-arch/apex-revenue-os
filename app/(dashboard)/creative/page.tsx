    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div><h3 className="text-lg font-semibold text-white">Generate Creative</h3><p className="text-sm text-slate-400 mt-0.5">AI-powered ad copy</p></div>
          <button onClick={onClose} className="text-slate-400 hover:text-white">X</button>
        </div>
        <div className="mb-4"><label className="block text-xs text-slate-400 mb-1">Vertical</label>
          <select value={vertical} onChange={e => setVertical(e.target.value as Vertical)} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm">
            {VERTICALS.map(v => <option key={v} value={v}>{v}</option>)}
          </select></div>
        <div className="mb-4"><label className="block text-xs text-slate-400 mb-1">Ad Type</label>
          <div className="grid grid-cols-3 gap-2">{AD_TYPES.map(t => (
            <button key={t} onClick={() => setType(t)} className={cn("py-2 rounded-lg text-sm font-medium capitalize border", type === t ? "bg-emerald-600 border-emerald-500 text-white" : "bg-slate-800 border-slate-700 text-slate-400")}>{t}</button>
          ))}</div></div>
        <div className="mb-6"><label className="block text-xs text-slate-400 mb-1">Context (optional)</label>
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={3} className="w-full bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm resize-none" /></div>
        {error && <div className="mb-4 p-2 bg-red-500/10 border border-red-500/30 rounded text-sm text-red-400">{error}</div>}
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2 rounded-lg border border-slate-700 text-slate-400 text-sm">Cancel</button>
          <button onClick={handleGenerate} disabled={loading} className="flex-1 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-sm">{loading ? "Generating..." : "Generate"}</button>
        </div>
      </div>
    </div>
  );
...
export default function CreativePage() {
  ...
  function handleStatus(id: string, status: Status) { store.update<Creative>(KEY, id, { status }); load(); }
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[{ l: "Total Impressions", v: fmt(stats.ti), c: "text-blue-400" }, { l: "Total Clicks", v: fmt(stats.tc), c: "text-emerald-400" }, { l: "Avg Predicted CTR", v: `${stats.ap.toFixed(2)}%`, c: "text-violet-400" }, { l: "Avg Actual CTR", v: `${stats.aa.toFixed(2)}%`, c: "text-amber-400" }].map(s => (
            <Card key={s.l} className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-5">
              <div className="text-xs text-slate-500 uppercase tracking-wide mb-2">{s.l}</div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as AdType | "all")} className="bg-slate-800 border border-slate-700 text-white rounded-lg px-3 py-2.5 text-sm">
            <option value="all">All Types</option>{AD_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
